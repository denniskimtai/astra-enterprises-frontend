  ...
  <div class="space-y-6 pb-6">
    <div class="px-6 py-6">
      <div class="flex items-baseline gap-2">
        <h1 class="text-2xl font-bold text-[#1A1A2E]">Active Customers</h1>
    ...
    <div v-if="loading" class="mx-6 py-12 flex justify-center items-center bg-white border border-[#E5E0EA] rounded-lg">
      <div class="flex flex-col items-center gap-3">
        ...
        <p class="text-sm text-[#4B4B6B]">Loading active customers...</p>
      </div>
    </div>
        ...
        <table class="min-w-full border-separate border-spacing-0">
          <thead class="bg-[#F8F7FA] border-b border-[#E5E0EA]">
            <tr>
              ...
              <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-[#4F1964]">Current Limit</th>
          ...
          <tbody>
            <tr v-if="customers.length === 0">
              <td colspan="10" class="px-4 py-8 text-center text-sm text-[#9CA3AF]">No active customers found.</td>
            </tr>
            <tr
            ...
            >
              ...
              <td class="px-4 py-3 text-sm text-[#374151]">
                <div class="h-10 w-10 overflow-hidden rounded-full ring-1 ring-[#E5E0EA]">
                  ...
                  >
                    {{ customer.fullName.split(' ').map(name => name[0]).join('') }}
                  </div>
                </div>
              </td>
              <!-- Name -->
              <td class="px-4 py-3 text-sm text-[#374151]">
                <span class="text-[#1A1A2E] font-semibold text-sm">{{ customer.fullName }}</span>
              </td>
              <!-- National ID -->
              ...
              <!-- Phone -->
              <td class="px-4 py-3 text-sm text-[#374151]">
                <span class="text-[#6B2385] text-xs underline cursor-pointer hover:text-[#4F1964] font-medium">{{ customer.phone }}</span>
              </td>
              <!-- Branch -->
              <td class="px-4 py-3 text-sm text-[#374151] text-xs">
                {{ branchesMap[customer.branchId] || 'Branch (' + customer.branchId.slice(0, 4) + ')' }}
              </td>
              <!-- Limit -->
              <td class="px-4 py-3 text-sm text-[#374151] font-bold text-xs">
                {{ formatCurrency(customer.currentLimit) }}
              </td>
              <!-- Location -->
              <td class="px-4 py-3 text-sm text-[#374151]">
                <div class="text-[#4B4B6B] text-xs leading-relaxed max-w-xs truncate" :title="customer.physicalAddress">
              ...
              <!-- Status -->
              <td class="px-4 py-3 text-sm text-[#374151]">
                <span class="inline-flex rounded-sm bg-[#4F1964] px-2 py-0.5 text-[10px] font-bold uppercase text-white">
